...
check_environment () {
	...
	env_dir="/opt/meza/config/local-secret"
...
case "$1" in
	...
	deploy)
		...
		check_environment "$2"
		host_file="/opt/meza/config/local-secret/$2/hosts"
		...
		sudo -u meza-ansible ansible-playbook /opt/meza/src/playbooks/site.yml -i "$host_file" --extra-vars "env=$2" ${@:3}
